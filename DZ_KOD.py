...
def temperature():
    def kodek():
        ...
        if d < 4.2 or d > 7.8:
            ...
            sel1 = "Включен нагриватель" \
                "\nИсполяющие устройство обагреватель № " + \
                str(var.get())
            sel2 = str(bit1)
            sel3 = str(bit1)
            bit2 = bit1
    ...
    btn = Button(top, bg="OliveDrab2", text="Рассчитать",
                 width=46, command=kodek
                 )
    btn.grid(row=3, column=0, columnspan=2, pady=5, padx=5)

    lbl1 = Label(top, bg="dark orange", text="Код устройства",
                 width=20, height=1
                 )
...
but1 = Button(root, text="Температура", width=50, height=1,
              bg="cadet blue", fg="white", command=temperature
              )

but2 = Button(root, text="Концентрация", width=50, height=1,
              bg="cadet blue", fg="white", command=concentration
              )

but3 = Button(root, text="Плотность тока", width=50, height=1,
              bg="cadet blue", fg="white", command=current
              )
